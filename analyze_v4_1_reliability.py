...
INPUT_PATH = PROJECT_ROOT / "results" / "baseline_with_groundtruth_v4_1.csv"
BEST_CONFIG_PATHS = [
    PROJECT_ROOT / "results" / "summaries_v4_1" / "best_full_configuration_v4_1.csv",
    PROJECT_ROOT / "results" / "summaries_v4_1" / "robust_best_full_configuration_v4_1.csv",
]

SUMMARY_DIR = PROJECT_ROOT / "results" / "summaries_v4_1_reliability"
FIGURES_DIR = PROJECT_ROOT / "results" / "figures" / "v4_1_reliability"
INTENSITY_CACHE_PATH = SUMMARY_DIR / "roi_intensity_features_v4_1.csv"
ENRICHED_OUTPUT_PATH = SUMMARY_DIR / "baseline_with_groundtruth_v4_1_reliability_enriched.csv"
...
# Compute intensity features for a single ROI
...
def build_or_load_intensity_cache(df):
    ...
    if INTENSITY_CACHE_PATH.exists():
        ...
        if len(missing_paths) == 0:
            print("Loaded existing v4.1 intensity cache:")
...
def load_best_configuration():
    for path in BEST_CONFIG_PATHS:
        if path.exists():
            ...
            if len(best_df) > 0:
                ...
                print("Loaded best v4.1 configuration from:")
                print(path)
                print()
    ...
    raise FileNotFoundError(
        "Could not find robust_best_full_configuration_v4_1.csv "
        "or best_full_configuration_v4_1.csv. "
        "Run src/find_best_configuration_v4_1.py first."
    )
...
def save_snr_threshold_analysis(df):
    thresholds = [-5, 0, 3, 6, 9, 12, 15]
    ...
    for threshold in thresholds:
        ...
        retained = successful[
            successful["median_snr_db"].notna()
            & (successful["median_snr_db"] >= threshold)
        ].copy()
        ...
        if n_retained > 0:
            ...
            signed_error = (
                retained["estimated_hr_bpm"]
                - retained["groundtruth_hr_bpm"]
            )

            mae = float(
                np.mean(np.abs(signed_error))
            )
    ...
    threshold_df.to_csv(
        SUMMARY_DIR / "snr_threshold_analysis_best_v4_1.csv",
        index=False,
    )
    ...
    plt.bar(
        threshold_df["snr_threshold_db"].astype(str),
        threshold_df["mae_bpm"],
    )

    plt.xlabel("SNR threshold (dB)")
    plt.ylabel("MAE (BPM)")
    plt.title("v4.1 MAE After Applying SNR Threshold")

    plt.tight_layout()

    plt.savefig(
        FIGURES_DIR / "mae_by_snr_threshold_v4_1.png",
        dpi=300,
    )
    ...
    plt.xlabel("SNR threshold (dB)")
    plt.ylabel("Coverage")
    plt.title("v4.1 Coverage After Applying SNR Threshold")
    ...
    plt.savefig(
        FIGURES_DIR / "coverage_by_snr_threshold_v4_1.png",
        dpi=300,
    )
...
def reconstruct_window_estimates(row):
    ...
    for start in starts:
        ...
        rows.append({
            ...
            "raw_peak_hr_bpm": best_result.get("raw_peak_hr_bpm", np.nan),
            "snr_db": best_result["snr_db"],
            "selected_component": best_component,
            "raw_frequency_resolution_bpm": best_result.get("raw_frequency_resolution_bpm", np.nan),
            "padded_frequency_resolution_bpm": best_result.get("padded_frequency_resolution_bpm", np.nan),
            "interpolation_delta_bins": best_result.get("interpolation_delta_bins", np.nan),
        })
...
def main():
    if not INPUT_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run src/add_groundtruth_v4_1.py first."
        )

    df = pd.read_csv(INPUT_PATH)

    print("Loaded v4.1 results:")
    print(INPUT_PATH)
    print("Rows:", len(df))
    print()
    ...
    print("Saved enriched v4.1 reliability file:")
    ...
    summaries = pd.DataFrame([
        calculate_summary(df, "all_v4_1_rows"),
        calculate_summary(best_df, "best_v4_1_configuration"),
    ])

    summaries.to_csv(
        SUMMARY_DIR / "reliability_main_summary_v4_1.csv",
        index=False
    )
    ...
    correlation_df.to_csv(
        SUMMARY_DIR / "reliability_correlations_best_config_v4_1.csv",
        index=False
    )
    ...
    # --------------------------------------------------------
    snr_threshold_df = save_snr_threshold_analysis(
        successful_best
    )
    # --------------------------------------------------------
    # Main plots
    ...
    save_estimated_vs_groundtruth(
        successful_best,
        filename="estimated_vs_groundtruth_best_v4_1.png",
        title="v4.1 Best Configuration: Estimated HR vs Ground Truth",
    )
    ...
    save_scatter(
        ...
        filename="absolute_error_vs_snr_best_v4_1.png",
        title="v4.1 Absolute Error vs SNR",
    ...
    save_scatter(
        ...
        y_column="absolute_error_bpm",
        filename="absolute_error_vs_noise_proxy_best_v4_1.png",
        title="v4.1 Absolute Error vs Intensity Noise Proxy",
    ...
    save_scatter(
        ...
        y_column="absolute_error_bpm",
        filename="absolute_error_vs_mean_intensity_best_v4_1.png",
        title="v4.1 Absolute Error vs Mean ROI Intensity",
    ...
    save_scatter(
        ...
        y_column="absolute_error_bpm",
        filename="absolute_error_vs_window_hr_std_best_v4_1.png",
        title="v4.1 Absolute Error vs Window HR Variability",
    ...
    save_scatter(
        ...
        y_column="absolute_error_bpm",
        filename="absolute_error_vs_temporal_diff_noise_best_v4_1.png",
        title="v4.1 Absolute Error vs Frame-to-Frame Intensity Instability",
    ...
    save_scatter(
        ...
        filename="noise_proxy_vs_snr_best_v4_1.png",
        title="v4.1 Intensity Noise Proxy vs SNR",
        xlabel="Median SNR (dB)",
        ylabel="Noise proxy",
    )

    # v4.1-specific diagnostic plots
    if "mean_abs_interpolation_delta_bins" in successful_best.columns:
        save_scatter(
            successful_best,
            x_column="mean_abs_interpolation_delta_bins",
            y_column="absolute_error_bpm",
            filename="absolute_error_vs_interpolation_delta_best_v4_1.png",
            title="v4.1 Absolute Error vs Interpolation Delta",
            xlabel="Mean absolute interpolation delta (FFT bins)",
            ylabel="Absolute error (BPM)",
        )

    if "median_padded_frequency_resolution_bpm" in successful_best.columns:
        save_scatter(
            successful_best,
            x_column="median_padded_frequency_resolution_bpm",
            y_column="absolute_error_bpm",
            filename="absolute_error_vs_padded_resolution_best_v4_1.png",
            title="v4.1 Absolute Error vs Padded FFT Resolution",
            xlabel="Median padded frequency resolution (BPM)",
            ylabel="Absolute error (BPM)",
        )
    ...
    save_binned_mae_plot(
        successful_best,
        x_column="median_snr_db",
        filename="binned_mae_by_snr_best_v4_1.png",
        title="v4.1 MAE by SNR Bin",
    ...
    save_binned_mae_plot(
        successful_best,
        x_column="noise_proxy_rgb",
        filename="binned_mae_by_noise_proxy_best_v4_1.png",
        title="v4.1 MAE by Intensity Noise Proxy Bin",
    ...
    save_binned_mae_plot(
        successful_best,
        x_column="mean_rgb_intensity",
        filename="binned_mae_by_mean_intensity_best_v4_1.png",
        title="v4.1 MAE by Mean ROI Intensity Bin",
    ...
    save_binned_mae_plot(
        successful_best,
        x_column="std_window_hr_bpm",
        filename="binned_mae_by_window_hr_std_best_v4_1.png",
        title="v4.1 MAE by Window HR Variability Bin",
    ...
    if "mean_abs_interpolation_delta_bins" in successful_best.columns:
        save_binned_mae_plot(
            successful_best,
            x_column="mean_abs_interpolation_delta_bins",
            filename="binned_mae_by_interpolation_delta_best_v4_1.png",
            title="v4.1 MAE by Interpolation Delta Bin",
            xlabel="Mean absolute interpolation delta per bin",
        )
    ...
    if len(successful_best) > 0:
        ...
        save_window_trace(
            typical_row,
            filename="window_hr_trace_typical_best_v4_1.png",
            title=(
                "Typical v4.1 Window-Level HR Trace\n"
        ...
        save_window_trace(
            worst_row,
            filename="window_hr_trace_worst_best_v4_1.png",
            title=(
                "Worst v4.1 Window-Level HR Trace\n"
