...
OUTPUT_PATH = PROJECT_ROOT / "results" / "baseline_rppg_results_v4_1.csv"
...
# v4.1 improvement:
# A 5-second FFT window has raw frequency resolution of about 12 BPM.
# Zero-padding creates a finer FFT grid.
# Parabolic interpolation refines the peak location between neighbouring bins.
ZERO_PADDING_FACTOR = 16
USE_PARABOLIC_INTERPOLATION = True

HR_ESTIMATION_METHOD = "windowed_fft_snr_component_median_interp"
...
# Detrend, normalise, and bandpass-filter one signal.
...
# ============================================================
# FFT + SNR estimation with zero-padding and interpolation
# ============================================================
# Get the next power of two for zero-padding
def next_power_of_two(x: int) -> int:
    return 1 if x <= 1 else 2 ** int(np.ceil(np.log2(x)))

# Parabolic peak interpolation
def parabolic_peak_interpolation(power_values: np.ndarray, peak_index: int):
    if peak_index <= 0 or peak_index >= len(power_values) - 1:
        return 0.0, float(power_values[peak_index])

    alpha = float(power_values[peak_index - 1])
    beta = float(power_values[peak_index])
    gamma = float(power_values[peak_index + 1])
    ...
    # Avoid unstable jumps.
    delta = float(np.clip(delta, -1.0, 1.0))

    interpolated_power = beta - 0.25 * (alpha - gamma) * delta

    return delta, float(interpolated_power)

# Estimate heart rate using FFT and SNR
def estimate_hr_fft_with_snr(
    ...
    raw_frequency_resolution_hz = fps / n
    raw_frequency_resolution_bpm = raw_frequency_resolution_hz * 60.0

    n_fft = next_power_of_two(n * ZERO_PADDING_FACTOR)

    freqs = np.fft.rfftfreq(n_fft, d=1.0 / fps)
    power = np.abs(np.fft.rfft(x_windowed, n=n_fft)) ** 2

    padded_frequency_resolution_hz = fps / n_fft
    padded_frequency_resolution_bpm = padded_frequency_resolution_hz * 60.0
    ...
    raw_peak_frequency_hz = valid_freqs[peak_index]
    raw_peak_hr_bpm = raw_peak_frequency_hz * 60.0
    raw_peak_power = float(valid_power[peak_index])

    interpolation_delta_bins = 0.0
    interpolated_peak_power = raw_peak_power

    if USE_PARABOLIC_INTERPOLATION:
        interpolation_delta_bins, interpolated_peak_power = parabolic_peak_interpolation(
            valid_power,
            peak_index,
        )

    frequency_step_hz = valid_freqs[1] - valid_freqs[0]

    interpolated_peak_frequency_hz = (
        raw_peak_frequency_hz
        + interpolation_delta_bins * frequency_step_hz
    )

    interpolated_peak_frequency_hz = float(
        np.clip(interpolated_peak_frequency_hz, low_hz, high_hz)
    )

    estimated_hr_bpm = interpolated_peak_frequency_hz * 60.0
    ...
    snr = interpolated_peak_power / (noise_power + 1e-12)
    snr_db = 10.0 * np.log10(snr + 1e-12)

    return {
        "estimated_hr_bpm": float(estimated_hr_bpm),

        "peak_frequency_hz": float(interpolated_peak_frequency_hz),
        "raw_peak_frequency_hz": float(raw_peak_frequency_hz),
        "raw_peak_hr_bpm": float(raw_peak_hr_bpm),

        "peak_power": float(interpolated_peak_power),
        "raw_peak_power": float(raw_peak_power),
        ...
        "n_fft": int(n_fft),
        "raw_frequency_resolution_bpm": float(raw_frequency_resolution_bpm),
        "padded_frequency_resolution_bpm": float(padded_frequency_resolution_bpm),
        "interpolation_delta_bins": float(interpolation_delta_bins),
    }
...
def estimate_recording_hr_windowed_snr(
    ...
    raw_hr_values = np.asarray([
        item["raw_peak_hr_bpm"]
        for item in window_estimates
    ])
    ...
    raw_resolution_values = np.asarray([
        item["raw_frequency_resolution_bpm"]
        for item in window_estimates
    ])

    padded_resolution_values = np.asarray([
        item["padded_frequency_resolution_bpm"]
        for item in window_estimates
    ])

    interpolation_delta_values = np.asarray([
        item["interpolation_delta_bins"]
        for item in window_estimates
    ])
    ...
    return {
        ...
        "median_raw_window_hr_bpm": float(np.median(raw_hr_values)),
        "mean_raw_window_hr_bpm": float(np.mean(raw_hr_values)),
        ...
        "median_raw_frequency_resolution_bpm": float(np.median(raw_resolution_values)),
        "median_padded_frequency_resolution_bpm": float(np.median(padded_resolution_values)),
        "mean_abs_interpolation_delta_bins": float(np.mean(np.abs(interpolation_delta_values))),
                ...
                for overlap in OVERLAPS:
                    ...
                    try:
                        ...
                        results.append({
                            ...
                            "median_raw_window_hr_bpm": hr_result["median_raw_window_hr_bpm"],
                            "mean_raw_window_hr_bpm": hr_result["mean_raw_window_hr_bpm"],
                            ...
                            "median_raw_frequency_resolution_bpm": hr_result["median_raw_frequency_resolution_bpm"],
                            "median_padded_frequency_resolution_bpm": hr_result["median_padded_frequency_resolution_bpm"],
                            "mean_abs_interpolation_delta_bins": hr_result["mean_abs_interpolation_delta_bins"],
                        ...
                        results.append({
                            ...
                            "median_raw_window_hr_bpm": np.nan,
                            "mean_raw_window_hr_bpm": np.nan,
                            ...
                            "median_raw_frequency_resolution_bpm": np.nan,
                            "median_padded_frequency_resolution_bpm": np.nan,
                            "mean_abs_interpolation_delta_bins": np.nan,
...
def main():
    ...
    print("Zero-padding factor:", ZERO_PADDING_FACTOR)
    print("Parabolic interpolation:", USE_PARABOLIC_INTERPOLATION)
    print("HR estimation method:", HR_ESTIMATION_METHOD)
    print()

    print(f"Usable rows for v4.1: {len(usable_rows)}")
    print()
    ...
    print()
    print("Saved v4.1 results to:")
    print(OUTPUT_PATH)
    print()

    print("v4.1 result summary")
    print("-------------------")
